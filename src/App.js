...
class App extends Component {
    ...
    try {
      ...
      const config = {
        headers: {
          ...
          "Ocp-Apim-Subscription-Key": "0ff905437c40417ca09c61be12c56748"
        }
      };

      const response = await axios.post(
        `http://192.168.0.105:5000/face/v1.0/detect?overload=stream&returnFaceId=true&returnFaceLandmarks=false&recognitionModel=recognition_02&returnRecgnitionModel=true`,
        blob,
        config
      );
  ...
  async identifyFaceInPersonGroup(data) {
    try {
      ...
      const response = await axios.post(
        "http://192.168.0.105:5000/face/v1.0/identify",
        payload,
        config
      );
  ...
  async getPersonData(personId, personGroupId = "my_friends") {
    try {
      ...
      const response = await axios.get(
        `http://192.168.0.105:5000/face/v1.0/persongroups/${personGroupId}/persons/${personId}`,
        config
      );
